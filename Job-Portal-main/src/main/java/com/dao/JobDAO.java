...
public class JobDAO {
	...
	public boolean updateJob(Jobs j) {
		...
		try {
			String sql = "update job_portal.jobs set title=?, discription=?, category=?, status=?, location=? where id = ?";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, j.getTitle());
			ps.setString(2, j.getDescription());
			ps.setString(3, j.getCategory());
	...
	public List<Jobs> getJobsOrLocationCategory(String cat, String loc) {
		...
		try {
			String sql = "select * from job_portal.jobs where status=? and category=? or location=? order by id desc";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, "Active");
			ps.setString(2, cat);
			ps.setString(3, loc);
	...
	public List<Jobs> getJobsAndLocationCategory(String cat, String loc) {
		...
		try {
			String sql = "select * from job_portal.jobs where status=? and category=? and location=? order by id desc";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, "Active");
			ps.setString(2, cat);
			ps.setString(3, loc);
